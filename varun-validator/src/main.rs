...
use solana_core::tvu::{Tvu, TvuConfig, TvuSockets, VarunTvu, VarunTvuConfig, VarunTvuSockets};
...
pub fn main123() {
    ...
    let logfile = {
        let logfile = matches
            .value_of("logfile")
            .map(|s| s.into())
            .unwrap_or_else(|| format!("varun-validator-{}.log", identity_keypair.pubkey()));
